...
void test_small_deltas() {
    ...
    std::cout << "Small Delta Test Passed! Compressed size: " << bits.size() << " bytes (Original: " << frame.size() * 2 << " bytes)" << std::endl;
}

void test_large_deltas() {
    int channels = 4;
    int samples = 10;
    myelin::BSDE encoder(channels);
    myelin::BSDE decoder(channels);
    ...
    for (int i = 0; i < frame.size(); ++i) {
        // Create large jumps
        frame[i] = (i % 2 == 0) ? 500 : 800; // Large deltas (300/-300)
    }
    ...
    std::cout << "Large Delta Test Passed! Compressed size: " << bits.size() << " bytes (Original: " << frame.size() * 2 << " bytes)" << std::endl;
}

int main() {
    test_small_deltas();
    test_large_deltas();
    std::cout << "All BSDE Core Tests Passed!" << std::endl;
    return 0;
}
